...
from PyQt5.QtWidgets import (QLabel, QPushButton, QComboBox, QWidget,
                             QVBoxLayout, QHBoxLayout, QFrame, QToolButton,
...
class LoadingIndicator(QLabel):
    """Animated GIF loading indicator that retains its space when hidden."""
    ...
    def __init__(self, parent=None):
        ...
        self.setStyleSheet("QLabel { background-color: transparent; padding: 4px 0px; }")
...
class BandComboBox(QComboBox):
    """
    Flat, Photoshop-style dropdown for band selection.
    Fixed-width when closed; wide enough for the longest item when open.
    ...
    def showPopup(self):
        ...
        if popup is None:
            return
        # size to longest item before computing position
        fm    = self.fontMetrics()
        max_w = max(
...
class CollapsibleSection(QWidget):
    """Premiere Pro-style collapsible section with animated arrow."""
